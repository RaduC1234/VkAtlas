...
def build_dfd() -> bytes:
    desc_block_size = 24 + 4 * 16
    buf = bytearray()
    buf += struct.pack("<I", 0)
    buf += struct.pack("<H", 2)
    buf += struct.pack("<H", desc_block_size)
    buf += struct.pack("B", 0)    # colorModel = RGBSDA
    ...
    for ch_id, bit_offset in [(0, 0), (1, 32), (2, 64), (15, 96)]:
        buf += struct.pack("<H", bit_offset)
        buf += struct.pack("B",  31)
        buf += struct.pack("B",  (ch_id & 0x0F) | 0xC0)   # float + signed
        buf += struct.pack("<I", 0)
        buf += struct.pack("<I", 0)
        buf += struct.pack("<I", 0x7F800000)
    return struct.pack("<I", 4 + desc_block_size) + bytes(buf)
...
def write_ktx2(mip_faces: list, out_path: str) -> None:
    ...
    mip_bytes_rev = list(reversed(mip_bytes))   # KTX2: smallest mip first
    ...
    out += struct.pack("<IIII",  dfd_offset, len(dfd), kvd_offset, 0)
    out += struct.pack("<QQ",    0, 0)
    assert len(out) == HEADER_SIZE

    for i, mb in enumerate(mip_bytes_rev):
        out += struct.pack("<QQQ", mip_offsets[i], len(mb), len(mb))
