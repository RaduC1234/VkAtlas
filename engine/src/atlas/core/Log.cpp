...
namespace Atlas {
    ...
    void setConsoleColor(LogLevel level) {
        ...
        switch (level) {
            case LogLevel::Trace: color = 7;  break;  // White
    ...
    void Log::log(LogLevel level, const std::string &message) {
        ...
        std::string logPrefix = getLogPrefix(level);
        ...
        __android_log_print(priority, "Atlas", "%s: %s", logPrefix.c_str(), message.c_str());
#else
#ifdef _WIN32
        setConsoleColor(level);
        std::cout << logPrefix << " " << message << std::endl;
        setConsoleColor(LogLevel::Trace); // Reset to default
#else
